...
export const mutateSavingsPlanTool = createTool({
  ...
  inputSchema: z.object({
    action: z.enum(["create_bucket", "set_bucket_contribution", "reallocate_monthly_fixed", "set_general_contribution"]),
  ...
  execute: async (input, context) => {
    ...
    const workflow = context.mastra?.getWorkflow("mutateSavingsPlan") ?? mutateSavingsPlan;
    ...
    return {
      ...
      message: result.status === "failed" ? result.error.message : "Savings plan mutation did not complete.",
    };
  },
});
