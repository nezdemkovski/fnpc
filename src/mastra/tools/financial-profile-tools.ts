...
import { getUserProfile, updateUserPreferences } from "../../finance/user-preferences";
...
const missingProfileSettings = (user: ProfileSettings): string[] => {
...
const userPreferencesSchema = z.object({
  ...
  responseLanguage: z.string().optional().describe("BCP-47 or short language code, for example en, es, de"),
...
export const saveFinancialFactsTool = createTool({
  ...
  execute: async (input, context) => {
    ...
    const workflow = context.mastra?.getWorkflow("updateFinancialProfile") ?? updateFinancialProfile;
    ...
    return {
      ...
      message: result.status === "failed" ? result.error.message : "Financial profile update did not complete.",
    };
  },
});
  ...
  execute: async (input, context) => {
    ...
    if (missingProfileFields.length > 0) {
      ...
      return {
        ...
        message: "Cannot calculate a financial snapshot until defaultCurrency and timezone are known.",
      };
    }
    ...
    return {
      ...
      formatted: {
        ...
        protectedSavings: formatMoney(snapshot.totals.protectedSavingsMinor, currency),
        availableOperatingCash: formatMoney(snapshot.totals.availableOperatingCashMinor, currency),
        monthlyIncome: formatMoney(snapshot.totals.monthlyIncomeMinor, currency),
        monthlyRecurringExpenses: formatMoney(snapshot.totals.monthlyRecurringExpensesMinor, currency),
        monthlySavingsContributions: formatMoney(snapshot.totals.monthlySavingsContributionsMinor, currency),
        monthlySurplus: formatMoney(snapshot.totals.monthlySurplusMinor, currency),
      },
      counts: {
  ...
  execute: async (input, context) => {
    ...
    if (missingProfileFields.length > 0) {
      return {
        ...
        ...currentContext(user),
        missingProfileFields,
        message: "Cannot run a forecast until defaultCurrency and timezone are known.",
      };
    }
    const currency = user.defaultCurrency;
    const timezone = user.timezone;
    ...
    const forecast = await runForecast({ userId: user.id, horizonMonths: input.horizonMonths ?? 6 });
  ...
  execute: async (input, context) => {
    ...
    if (missingProfileFields.length > 0) {
      return {
        ...
        ...currentContext(user),
        missingProfileFields,
        message: "Cannot run a purchase scenario until defaultCurrency and timezone are known.",
      };
    }
    const currency = user.defaultCurrency;
    const timezone = user.timezone;
    ...
    return {
      ...
      formatted: {
        ...
        rows: forecast.rows.map((row) => ({
          ...
          income: formatMoney(row.incomeMinor, currency),
          recurringExpenses: formatMoney(row.recurringExpensesMinor, currency),
          plannedAndScenarioExpenses: formatMoney(row.plannedExpensesMinor, currency),
          actualExpenses: formatMoney(row.actualExpensesMinor, currency),
          savingsContributions: formatMoney(row.savingsContributionsMinor, currency),
          closingFreeCash: formatMoney(row.closingFreeCashMinor, currency),
          protectedSavings: formatMoney(row.protectedSavingsMinor, currency),
